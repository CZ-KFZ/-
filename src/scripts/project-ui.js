...
export function openProjectModal(project) {
  ...
  const safeCover = safeUrl(project.coverImage)
  const coverHtml = safeCover
    ? `<div class="mb-6 rounded-[var(--evo-radius-md)] overflow-hidden aspect-[16/9]"><img src="${safeCover}" alt="${safeTitle}" class="w-full h-full object-cover" loading="lazy" /></div>`
    : `<div class="mb-6 rounded-[var(--evo-radius-md)] h-48 sm:h-64 bg-gradient-to-br ${gradient} flex items-center justify-center p-4"><span class="evo-title text-2xl sm:text-3xl text-white/90 text-center">${safeTitle}</span></div>`
  ...
  if (xiguaId) {
    mediaHtml = `
      <div class="mb-6 rounded-[var(--evo-radius-md)] overflow-hidden aspect-video bg-black">
        <iframe width="100%" height="100%" src="https://www.ixigua.com/iframe/${escapeHtml(xiguaId)}?autoplay=0" title="${safeTitle} 视频" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen referrerpolicy="unsafe-url"></iframe>
      </div>`
  } else if (ytId) {
    mediaHtml = `
      <div class="mb-6 rounded-[var(--evo-radius-md)] overflow-hidden aspect-video bg-black">
        <iframe width="100%" height="100%" src="https://www.youtube.com/embed/${escapeHtml(ytId)}?rel=0&modestbranding=1" title="${safeTitle} 视频" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
      </div>`
  } else if (project.video) {
    const safeVideo = safeUrl(project.video)
    if (safeVideo) {
      mediaHtml = `
        <div class="mb-6 rounded-[var(--evo-radius-md)] overflow-hidden">
          <video src="${safeVideo}" controls class="w-full"></video>
        </div>`
    }
  }

  let actionHtml = ''
  const safeDemoUrl = safeUrl(project.demoUrl)
  if (safeDemoUrl) {
    if (xiguaId) {
      actionHtml = `
        <div class="flex flex-wrap gap-3 mt-6">
          <a href="${safeDemoUrl}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-[var(--evo-radius-md)] border border-[var(--evo-border)] text-[var(--evo-ink-2)] hover:text-white hover:border-[var(--evo-purple-400)] transition-all text-sm">
      ...
      actionHtml = `
        <a href="${safeDemoUrl}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-6 py-3 rounded-[var(--evo-radius-md)] bg-gradient-to-r from-[var(--evo-purple-600)] to-[var(--evo-violet)] text-white hover:brightness-110 transition-all shadow-lg shadow-[var(--evo-purple-600)]/20 mt-6">
  ...
  modal.innerHTML = `
    <div class="evo-glass max-w-2xl w-full max-h-[85vh] overflow-y-auto rounded-[var(--evo-radius-lg)] p-6 md:p-10 relative" onclick="event.stopPropagation()">
      ...
      <div class="flex flex-wrap items-center gap-2 mb-4">
        <span class="px-2 py-1 rounded-[var(--evo-radius-sm)] ${toneCls} text-xs">${escapeHtml(project.categoryLabel || project.category || '')}</span>
        <span class="text-xs text-[var(--evo-ink-3)]">${escapeHtml(project.year || '')}</span>
      </div>
      <h1 class="evo-title text-2xl sm:text-3xl mb-6">${safeTitle}</h1>
      <div class="text-[var(--evo-ink-2)] leading-loose mb-6 whitespace-pre-line">${escapeHtml(project.desc || '暂无介绍')}</div>
      ${mediaHtml}
      ${actionHtml}
    </div>
  `
